...
for e in range(nentries):
    ...
    N = h4.Draw(f'{dtime}:{phase}: trg ==PHYS && evt_flag && fabs(h1X.clusters.X_- {Cx}) < 5 && fabs(h1Y.clusters.Y_ - {Cy} ) < 5  && digi_t.amp_max[C2_T]>{args.minamp} && fit_ampl[MCP1]>200&& digi_t.amp_max[C2_T]<{args.maxamp}','','goff', 1, e)
    #N = h4.Draw(f'{dtime}:{phase}: trg ==PHYS && n_tracks==1 && fabs(X)<5 && fabs(Y+5)<5 && digi_t.amp_max[C2_T]>{args.minamp} && fit_ampl[MCP1]>120&& digi_t.amp_max[C2_T]<{args.maxamp}','','goff', 1, e)
    dtime = h4.GetVal(0)[0]
    phase = h4.GetVal(1)[0]
    ...
    if N > 0 :
        ...
        WF_time = h4.GetVal(1)
        noZeros = True 
        correction = (digi_t.time_max[h4.C2_T]-corr.GetBinContent(corr.FindBin(digi_t.time_max[h4.C2_T]-int(digi_t.time_max[h4.C2_T]/6.238)*6.238))) 
        max_time = digi.time_max[h4.C2]- correction
        #max_t_time = digi_t.time_max[h4.C2_T]-(digi_t.time_max[h4.C2_T]-corr.GetBinContent(corr.FindBin(digi_t.time_max[h4.C2_T]-int(digi_t.time_max[h4.C2_T]/6.238)*6.238)))
        if (max_time <155 or max_time > 170): noZeros = False 
         
        print(f'EV : {e} phase = {digi_t.time_max[h4.C2_T]-int(digi_t.time_max[h4.C2_T]/6.238)*6.238}') 
        if (noZeros):
            ...
            for i in range (25, 55):
                ...
                ampl = WF_val[i]
                #phase = digi_t.time_max[h4.C2_T]-int(digi_t.time_max[h4.C2_T]/6.238)*6.238
                #print(f'EV : {e} t = {corr_time}  A = {ampl} corr = {correction}  phase = {digi_t.time_max[h4.C2_T]-int(digi_t.time_max[h4.C2_T]/6.238)*6.238}')
